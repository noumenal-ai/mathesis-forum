...
export function titleOf(doc, declName) {
  const m = /^\s*\*\*([^*]+)\*\*/.exec(doc ?? "");
  // A title is a name, not a sentence: a lead written as one loses its stop.
  return m ? m[1].trim().replace(/(?<!\.)\.$/, "") : declName.split(".").pop();
}
...
/** The body of the GitHub discussion that holds a post's thread: whose
 *  argument it is (the discussion's title already names it), their words,
 *  the statement, and the way back to the record. */
export function threadBody(post, ctx) {
  const lead = (post.doc ?? "").split(/\n\s*\n/)[0].trim();
  const who = post.author.name ?? post.author.login;
  return [
    `[**${who}**](${ctx.mathesis}/u/${encodeURIComponent(post.author.login)}/)`,
    lead,
    "```lean\n" + post.statement + "\n```",
