 ...
 *  opens threads there; anyone signed in to GitHub can reply. A thread is opened
 *  once: GitHub lets the workflow's token open a discussion but not edit one, so
 *  its opening post stays as written, while the forum's page follows the post. */
async function openThreads(posts, threads) {
  const d = await gql(
...
if (env.GITHUB_TOKEN) {
  ...
  if (env.CREATE_THREADS === "1") await openThreads(posts, threads);
} else {
